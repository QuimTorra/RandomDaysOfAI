class NeuralLayer {
  ...
  forward (input) {
    ...
    for (let i = 0; i < result[0].length; i++) {
      result[1][i] = this.act_f(result[0][i]);
    }
...
class NeuralNetwork {
  ...
  constructor (layers, act_f) {
    this.layers = [];
    this.outputNeurons = layers[layers.length-1];
    var nn = layers[0];
    var pn = nn;
  ...
  /**
   * @param {Array: Object} dataSet An array of objects containing an Input field and an Output field
   */
  learn (dataSet) {
    for (let s = 0; s < dataSet.length; s++) {
      // Forward Pass
      var input = dataSet[s].input;
      var expected = dataSet[s].output;
      ...
      for (let i = this.layers.length; i > 0; i--) {
        ...
        if (i == this.layers.length) {
          deltas.unshift(this.der_cost(expected, output) * this.layers[i].act_f_der())
          continue;
        }
